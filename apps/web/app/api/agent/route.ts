...
export async function POST(req: Request) {
  ...
  if (cl > 64 * 1024) return NextResponse.json({ ok: false, error: "payload_too_large" }, { status: 413 });
  ...
  try {
    ...
    return NextResponse.json({ ok: true, content: text, transcript_id: body.transcript_id });
  } catch (e) {
    console.error("[agent] failed", e);
    return NextResponse.json({ ok: false, content: "Connection error. Try again or email portfolio@concierge.studio." });
  }
}
